...
from toyssh.crypto_utils import decrypt_json_bytes, encrypt_json_bytes
...
def send_encrypted_json(sock: socket.socket, session_key: bytes, message: dict[str, Any]) -> None:
    ...
    send_frame(sock, encrypt_json_bytes(session_key, plaintext))


def recv_encrypted_json(sock: socket.socket, session_key: bytes) -> dict[str, Any]:
    payload = recv_frame(sock)
    plaintext = decrypt_json_bytes(session_key, payload)
